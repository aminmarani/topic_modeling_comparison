...
#  ans = subprocess.run(['python3', 'iteration_analysis.py'], stdout=subprocess.PIPE,stderr=subprocess.STDOUT).stdout.decode('utf-8')
  ...
  if technique == 'lda':
    ldaMallet = LdaMallet(mallet_path, corpus=doc_term_matrix, num_topics=num_topics, id2word=vocab_dict,iterations=iterations,random_seed = random_seed,optimize_interval=optimize_interval,alpha=alpha,workers=3)
