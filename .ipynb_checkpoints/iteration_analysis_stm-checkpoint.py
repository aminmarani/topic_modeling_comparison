...
#converting into format that is appropriate for STM
text_df = pd.DataFrame(zip(np.arange(1,len(doc_list)),doc_list),columns=['int_values','text'])
robjects.r.source('stm.R')

for t_num in topic_num:
    for _ in range(1): #three runs
        ans = robjects.r.run_stm(text_df,topic_n=t_num,max_itr=iterations,verbose=True)
        with open('t_num'+str(np.random.randint(100000))+'stm.obj','wb') as stm_file:
            pickle.dump(ans,stm_file)

 

print('STM runs are finished!')
exit()
